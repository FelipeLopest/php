...
if (isset($_POST['id_alterar'])) {
    $cx = conectar();
    $sql = "UPDATE emprestimo SET id_usuario = :id_usuario, id_livro = :id_livro, data_emprestimo = :data_emprestimo, data_devolucao = :data_devolucao, valor_aluguel = :valor_aluguel WHERE id = :id";
    $stmt = $cx->prepare($sql);
    $stmt->execute([
        ...
        <?php foreach ($alugueis as $aluguel): ?>
            <tr>
                ...
                <td>
                    ...
                    <a href="?pagina=verificar_aluguel&excluir=<?= $aluguel['id'] ?>" onclick="return confirm('Tem certeza que deseja excluir?')">Excluir</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
    ...
    <form method="post">
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" id="id_usuario" name="id_usuario" value="<?= $aluguel_editar['id_usuario'] ?>" required>
            </div>
            <div class="col-md-4">
                <label for="id_livro" class="form-label">ID Livro</label>
                <input type="text" class="form-control" id="id_livro" name="id_livro" value="<?= $aluguel_editar['id_livro'] ?>" required>
            </div>
            <div class="col-md-4">
                <label for="data_emprestimo" class="form-label">Data Empréstimo</label>
                <input type="date" class="form-control" id="data_emprestimo" name="data_emprestimo" value="<?= $aluguel_editar['data_emprestimo'] ?>" required>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col-md-4">
                <label for="data_devolucao" class="form-label">Data Devolução</label>
                <input type="date" class="form-control" id="data_devolucao" name="data_devolucao" value="<?= $aluguel_editar['data_devolucao'] ?>" required>
            </div>
            <div class="col-md-4">
                <label for="valor_aluguel" class="form-label">Valor</label>
                <input type="text" class="form-control" id="valor_aluguel" name="valor_aluguel" value="<?= $aluguel_editar['valor_aluguel'] ?>" required>
            </div>
        </div>
